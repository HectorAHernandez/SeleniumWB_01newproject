 ...
 */
/* Note this test Data provider INTERNAL (Below this code).
 * And must be ran from the external XML parameters file "TestNGDataProviderITestcontex.xml" 
 * Located in directory "SeleniumXML", because the groups are defined there as parameters, also there
 * is where it is indicated which tests are going to be included.
 * */
...
public class T26DataprovParamaterAsIContextGroupIntern {
	WebDriver driver;
	...
	// Each row in the groupArray defined in the dataProvider is passed to the method at execution time. 
